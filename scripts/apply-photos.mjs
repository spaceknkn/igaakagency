...
const targets = {
    'Vandalrock': '/artists/Vandalrock',
    'DJ Una': '/artists/DJ Una',
    'Juncoco': '/artists/Juncoco',
    'Smesher': '/artists/Smesher',
    'DJ Siro': '/artists/DJ Siro'
};

let updatedCount = 0;
for (const artist of artists) {
    if (targets[artist.name]) {
        const basePath = targets[artist.name];
        artist.photos = [
            `${basePath}/001.jpg`,
            `${basePath}/002.jpg`,
            `${basePath}/003.jpg`
...
if (updatedCount > 0) {
    ...
    console.log('No artists updated, check target names.');
}
